...
HTTPTracker.prototype.requestHandler = function () {
  ...
  return function _requestHandler(request, response) {
    ...
    if (request.headers['x-forwarded-for'] != null) {
      ...
      logger.debug('Trusting X-Forwarded-For header. Connecting IP is', request.connection.remoteAddress);
      remoteAddress = request.headers['x-forwarded-for'];
    } else {
      remoteAddress = request.connection.remoteAddress;
    }
    
    logger.debug('Announce request from', remoteAddress, query);
    ...
    try {
      ...
      peer = new trackerlib.Peer(remoteAddress, query.port, query.left);
